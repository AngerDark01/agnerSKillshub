...
3. For extraction/splitting tasks, inspect the boundary paragraphs/elements before cleanup.
   - A visually empty boundary paragraph may still carry a page break, drawing, or embedded object.
   - Prefer removing only the specific page break or trailing boundary artifact, not the whole paragraph, unless you confirmed it is truly empty.
   - Preserve intentional internal page breaks inside the extracted content.
4. After each meaningful change, re-render and inspect the pages.
   - For extracted chapters/tables, always inspect the first and last rendered pages for blank-page artifacts or clipped content.
...
- For standalone extracted files, explicitly check:
  - the first heading/table starts on page 1 instead of after a carried-over page break
  - the last page does not contain a boundary-only blank page
  - boundary cleanup did not remove images or embedded objects hidden in empty-looking paragraphs
